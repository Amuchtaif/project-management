<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
    <!-- Stats Cards -->
    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition">
        <div class="flex items-center justify-between mb-4">
            <div class="bg-purple-100 dark:bg-purple-900/30 p-3 rounded-2xl">
                <svg class="w-6 h-6 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2z"></path></svg>
            </div>
        </div>
        <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1"><?= ($data['user_role'] ?? 'admin') === 'admin' ? 'Total Proyek' : 'Proyek Saya'; ?></p>
        <h3 class="text-3xl font-bold text-gray-900 dark:text-gray-100"><?= $data['stats']['projects']; ?></h3>
    </div>

    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition">
        <div class="flex items-center justify-between mb-4">
            <div class="bg-blue-100 dark:bg-blue-900/30 p-3 rounded-2xl">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1"><?= ($data['user_role'] ?? 'admin') === 'admin' ? 'Tugas Aktif' : 'Tugas Saya'; ?></p>
        <h3 class="text-3xl font-bold text-gray-900 dark:text-gray-100"><?= $data['stats']['tasks']; ?></h3>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Tugas Selesai</p>
        <h3 class="text-3xl font-bold text-gray-900 dark:text-gray-100"><?= number_format($data['stats']['tasks'] * 0.8, 0); ?></h3>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Anggota Tim</p>
        <h3 class="text-3xl font-bold text-gray-900 dark:text-gray-100"><?= $data['stats']['members']; ?></h3>
    </div>
    <?php else : ?>
    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition">
        <div class="flex items-center justify-between mb-4">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Role Anda</p>
        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 capitalize"><?= $_SESSION['user']['role']; ?></h3>
    </div>
    <?php endif; ?>
</div>
        ...
        <div class="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm transition-colors">
            ...
            <div class="space-y-8">
                ...
                <div class="group/proj">
                    ...
                    <div class="relative w-full bg-gray-50 dark:bg-slate-700 h-2 rounded-full mb-4">
                        <div class="h-full rounded-full <?= $pColor ?> transition-all duration-700" style="width: <?= $progress; ?>%"></div>
                        <!-- Slider Look decoration -->
                        <div class="absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-white dark:bg-slate-900 border-2 <?= $borderCol ?> rounded-full shadow-sm shadow-black/10 transition-all duration-700 pointer-events-none" style="left: calc(<?= $progress; ?>% - 7px)"></div>
                    </div>
